...
public class ObjectPoint : MonoBehaviour //Can potentially migrate to Interactable
{
    ...
    private void OnTriggerEnter(Collider collider)
    {
        ...
        if (collider.CompareTag("Player") && TimeManager.instance.ReturnTime() >= 18 && objectType == 2 && TimeManager.instance.ReturnProgress() <= 2)//Bed
        {
            questIcon.SetActive(true);
            playerIsNear = true;
        }
        ...
        if (collider.CompareTag("Player") && GameObject.Find("VisitSpotsQuest(Clone)") && objectType == 3)//Interact
        {
            questIcon.SetActive(true);
            playerIsNear = true;
        }
